...
def handle_heal_use_event(battle, event):
...
def update_heal_use_phase(battle):
    # PHASE 1 — scroll "X uses Y!"
    if not battle.item_use_scroll_done:
        chars_per_second = battle.scroll_delay * 20
        chars_per_frame = chars_per_second / 60
...
def apply_item_healing(battle):
    ...
    heal = compute_heal_amount_item(battle, ally, item)
...
def apply_skill_healing(battle):
    move_name = battle.model.get_active_pokemon().moves[battle.skills_scroll + battle.skills_cursor]
    move = battle.smt_moves.get(move_name)
    ally = battle.model.player_team[battle.selected_ally]

    # 1) Compute heal amount
    heal = compute_heal_amount_skill(battle, ally, move)
...
def compute_heal_amount_item(battle, ally, item):
    heal_type = item["type"]
    amount = item["amount"]
...
def compute_heal_amount_skill(battle, ally, move):
    amount = move["power"]
    return int(ally.max_hp * (amount / 100))
