...
from smt_stats import load_smt_from_json
...
class PokemonController:
    ...
    def __init__(self):
        ...
        root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        json_path = os.path.join(root, "smt_stats.json")
        # Load Pokémon list
    ...
    def sort_by_stat(self, stat_name):
        """
        Sort Pokémon by a specific stat (hp, atk, defense, sp_atk, sp_def, speed).
        Highest first.
        """
    ...
    def sort_by_affinity(self, index):
        """
        Sort Pokémon by affinity value for a specific element index.
        Highest first.
        """
        self.pokemon_list.sort(
